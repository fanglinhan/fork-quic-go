...
import (
	...
	"github.com/fanglinhan/fork-quic-go/internal/congestion"
...
func (h *sentPacketHandler) ReceivedAck(ack *wire.AckFrame, encLevel protocol.EncryptionLevel, rcvTime time.Time) (bool /* contained 1-RTT packet */, error) {
	...
	if congestionEventHandler, ok := h.congestion.(congestion.CongestionEventParticular); ok &&
		(len(ackedPacketsInfo) != 0 || len(lostPacketsInfo) != 0) {
		congestionEventHandler.OnCongestionEventParticular(priorInFlight, rcvTime, ackedPacketsInfo, lostPacketsInfo)
	}
	...
	err := pnSpace.history.Iterate(func(p *packet) (bool, error) {
		...
		if packetLost {
			...
			if !p.skippedPacket {
				...
				lostPackets = append(lostPackets, protocol.LostPacketInfo{
					PacketNumber: p.PacketNumber,
					BytesLost:    p.Length,
				})
			}
	...
	if !earliestLossTime.IsZero() {
		...
		if err == nil {
			if congestionEventHandler, ok := h.congestion.(congestion.CongestionEventParticular); ok &&
				...
				congestionEventHandler.OnCongestionEventParticular(priorInFlight, time.Now(), nil, lostPacketsInfo)
			}
		}
		return err
	}
