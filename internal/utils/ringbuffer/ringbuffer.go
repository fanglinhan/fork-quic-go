...
func (r *RingBuffer[T]) PopFront() T {
	if r.Empty() {
		panic("github.com/fanglinhan/fork-quic-go/internal/utils/ringbuffer: pop from an empty queue")
	}
	r.full = false
	t := r.ring[r.headPos]
...
func (r *RingBuffer[T]) PeekFront() T {
	if r.Empty() {
		panic("github.com/fanglinhan/fork-quic-go/internal/utils/ringbuffer: peek from an empty queue")
	}
...
func (r *RingBuffer[T]) Offset(index int) *T {
	if r.Empty() || index >= r.Len() {
		panic("github.com/fanglinhan/fork-quic-go/internal/utils/ringbuffer: offset from invalid index")
	}
...
func (r *RingBuffer[T]) Front() *T {
	if r.Empty() {
		panic("github.com/fanglinhan/fork-quic-go/internal/utils/ringbuffer: front from an empty queue")
	}
	return &r.ring[r.headPos]
}
...
func (r *RingBuffer[T]) Back() *T {
	if r.Empty() {
		panic("github.com/fanglinhan/fork-quic-go/internal/utils/ringbuffer: back from an empty queue")
	}
